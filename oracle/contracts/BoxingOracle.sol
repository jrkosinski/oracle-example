...
contract BoxingOracle is Ownable {
    ...
    function addMatch(
        ...
        bytes32 id = keccak256(
            abi.encodePacked(_name, _participantCount, _date)
        );
        ...
        matchIdToIndex[id] = matches.length;
    ...
    function declareOutcome(
        ...
        if (_outcome == MatchOutcome.Decided) theMatch.winner = _winner;
    }
    ...
    function getPendingMatches() public view returns (bytes32[] memory) {
        ...
        for (uint i = 0; i < matches.length; i++) {
            if (matches[i].outcome == MatchOutcome.Pending) count++;
        }
    ...
    /// @return id : match data of the specified match
    function getMatch(
        bytes32 _matchId
    )
        public
    ...
    function addTestData() external onlyOwner {
        addMatch(
            ...
            "Pacquiao|Mayweather",
            2,
            DateLib.DateTime(2018, 8, 13, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Macquiao vs. Payweather",
            "Macquiao|Payweather",
            2,
            DateLib.DateTime(2018, 8, 15, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Pacweather vs. Macarthur",
            "Pacweather|Macarthur",
            2,
            DateLib.DateTime(2018, 9, 3, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Macarthur vs. Truman",
            "Macarthur|Truman",
        ...
        addMatch(
            ...
            "Macaque|Pregunto",
            2,
            DateLib.DateTime(2018, 9, 21, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Farsworth vs. Wernstrom",
            "Farsworth|Wernstrom",
            2,
            DateLib.DateTime(2018, 9, 29, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Fortinbras vs. Hamlet",
            "Fortinbras|Hamlet",
            2,
            DateLib.DateTime(2018, 10, 10, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Foolicle vs. Pretendo",
            "Foolicle|Pretendo",
            2,
            DateLib.DateTime(2018, 11, 11, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
        addMatch(
            "Parthian vs. Scythian",
            "Parthian|Scythian",
            2,
            DateLib.DateTime(2018, 11, 12, 0, 0, 0, 0, 0).toUnixTimestamp()
        );
    }
}
